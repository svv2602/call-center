...
### 1.2 Бюджет задержки (latency budget)

Обязательный контракт — распределение бюджета 2000 мс по компонентам:

| Этап | Бюджет (мс) | Метрика | Алерт при |
|------|-------------|---------|-----------|
| AudioSocket → STT | ≤ 50 | `audiosocket_to_stt_ms` | > 100 мс |
| STT распознавание | ≤ 500 | `stt_latency_ms` | > 700 мс |
| LLM (TTFT) | ≤ 1000 | `llm_latency_ms` | > 1500 мс |
| TTS синтез | ≤ 400 | `tts_latency_ms` | > 600 мс |
| TTS → AudioSocket | ≤ 50 | `tts_delivery_ms` | > 100 мс |
| **Итого (end-to-end)** | **≤ 2000** | `total_response_latency_ms` | **> 2500 мс** |

**Правила:**

- Каждый компонент **обязан** укладываться в свой бюджет при нормальной нагрузке (p95).
- При превышении бюджета компонента — срабатывает алерт и создаётся задача на оптимизацию.
- Бюджет пересматривается при смене провайдера STT/TTS/LLM.
- Подробная диагностика при превышении: см. [troubleshooting.md — раздел 8.1](../development/troubleshooting.md#81-высокая-общая-задержка-ответа-2-сек).
...
| Обработка звонков | Аудио обрабатывается в реальном времени (streaming STT), НЕ сохраняется; хранятся только транскрипции (90 дней) |
...
| Уведомление об обработке | Бот сообщает что звонок может обрабатываться автоматизированной системой |
