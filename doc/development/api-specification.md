...
Каждый ответ API содержит заголовок `X-Request-Id` для корреляции логов и отладки.
...
| 429 | Слишком много запросов (rate limit). Заголовок `Retry-After` указывает время ожидания (сек) |
| 500 | Внутренняя ошибка |
| 503 | Сервис временно недоступен. Заголовок `Retry-After` указывает ожидаемое время восстановления |

### Ретраи и таймауты

| Параметр | Значение | Описание |
|----------|----------|----------|
| Таймаут запроса | 5 сек | Максимальное время ожидания ответа |
| Ретраи | до 2 | Только для 429 и 503 |
| Backoff | exponential | 1 сек → 2 сек |
| Ретраи при 500 | нет | 500 = ошибка логики, ретрай не поможет |
...
|-----------|---------------|----------|
| `Idempotency-Key` | **Рекомендуется** | Уникальный ключ для предотвращения дубликатов при ретраях. Формат: UUID. При повторном запросе с тем же ключом возвращается ранее созданный заказ (201 с тем же телом). Ключ действителен 24 часа. |
...
| Заголовок | Обязательность | Описание |
|-----------|---------------|----------|
| `Idempotency-Key` | **Рекомендуется** | Уникальный ключ. При повторном подтверждении с тем же ключом — 200 с текущим состоянием заказа (без повторного SMS). |
