...
- ТАК → запитай номер договору (або пошукай через get_customer_bookings)
  - Шини на складі → записуй НЕ РАНІШЕ ніж через 3 робочих дні, попередь клієнта
  - Шини в шинному центрі або привезе сам → продовжуй
- НІ → переходь далі
...
**Крок 7 — Запис:**
⚡ Виклич book_fitting зі зібраними даними (station_id, date, time, ім'я, телефон, номер авто, \
номер договору зберігання якщо є, linked_order_id якщо замовляв шини).
