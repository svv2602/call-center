...
"перенести"/"змінити час" = скасування + новий запис:
1. get_customer_bookings → підтверди → cancel_fitting
2. Нова дата/час → get_fitting_slots → Крок 8 → book_fitting
⚡ ВСІ дані (station_id, ім'я, авто) вже відомі з попереднього запису — НЕ ПИТАЙ повторно!
